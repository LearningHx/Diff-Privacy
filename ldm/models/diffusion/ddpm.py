...
class DDPM(pl.LightningModule):
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        #print("noise",noise[0,0,15,3:])
        #asd = default(None, lambda: torch.randn_like(x_start))
        #print("asd",asd[0,0,15,3:]) 
...
class LatentDiffusion(DDPM):
    ...
    def get_learned_conditioning(self, c, x = None,t=None):
        if self.cond_stage_forward is None:
            if hasattr(self.cond_stage_model, 'encode') and callable(self.cond_stage_model.encode):
                c = self.cond_stage_model.encode(c, embedding_manager=self.embedding_manager, input_img = x,time_steps=t)
        ...
        if self.model.conditioning_key is not None:
            ...
            if not self.cond_stage_trainable or force_c_encode:
                if isinstance(xc, dict) or isinstance(xc, list):
                    ...
                    c=[]
                    for i in range(10):
                        c.append(self.get_learned_conditioning(xc,x,i))
                    #if bs is not None:
                        #for i in range(10):
                            #c[i] = c[i][:bs]
                else:
    ...
    def forward(self, x, c, img, *args, **kwargs):
        t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=self.device).long()
        normalize_t = int(t*10/self.num_timesteps)
        if self.model.conditioning_key is not None:
            assert c is not None
            # c = text
            if self.cond_stage_trainable:
                c = self.get_learned_conditioning(c, img,normalize_t)
            # c = tensor embed
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        if sample:
            ...
            uc = []
            for i in range(10):
                uc.append(self.get_learned_conditioning(len(c[0]) * [""], x,i))
